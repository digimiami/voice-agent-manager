...
INVENTORY_TOOL = [{
    ...
    "async": False,
    "method": "POST",
    "url": "https://diazites.online/api/inventory/search",
    "function": {
        "name": "search_inventory",
        "description": "Search the dealership's LIVE vehicle inventory for available cars, trucks, and SUVs by make, model, year, price, or keywords. Use this whenever a customer asks what vehicles are available, prices, or anything in stock.",
        "parameters": {
            "type": "object",
            "properties": {
                "q": {"type": "string", "description": "Natural-language search, e.g. \"Mercedes SUV under 30k\" or \"2018 Ford Explorer\". Optional but recommended."},
                "make": {"type": "string", "description": "Vehicle make, e.g. Ford, Toyota, Honda"},
                "model": {"type": "string", "description": "Vehicle model, e.g. Explorer, Camry"},
                "yearMin": {"type": "integer", "description": "Earliest model year"},
                "yearMax": {"type": "integer", "description": "Latest model year"},
                "priceMin": {"type": "integer", "description": "Minimum price in dollars"},
                "priceMax": {"type": "integer", "description": "Maximum price in dollars"}
            }
        }
    }
}]
...
def api_signup():
    ...
    # Create business
    price_map = {'starter': 97, 'pro': 197, 'premium': 497, 'enterprise': 0, 'custom': 0}
...
def api_signup_stripe():
    ...
    try:
        ...
        if is_enterprise:
            ...
            url = create_stripe_checkout(bid, 'Enterprise', 0, email, success_url, cancel_url, trial_days=0, mode='setup')
        else:
...
def make_vapi_call(lead, biz, assistant_id, phone_id, call_delay):
    ...
    try:
        ...
        if agent_prompt or script or kb:
            ...
            # Append the outbound script — what to say when CALLING this lead
            outbound_script = biz.get('script_outbound') or ''
            if outbound_script.strip():
                full_prompt += (
                    "\n\n--- OUTBOUND CALLS (WHEN YOU CALL A PROSPECT OR LEAD — NOT WHEN THEY CALL YOU) ---\n"
                    + outbound_script.strip()
                )
